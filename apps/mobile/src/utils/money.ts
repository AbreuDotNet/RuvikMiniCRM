/**
 * Mirror of the API's `lib/money.ts`, for the quote builder's live preview.
 *
 ...
 * web mirror and the API once disagreed exactly like that: the API rounded
 * tax per line, the preview rounded the aggregate, and a discount shifted the
 * tax by a cent.
 *
 * `apps/api/tests/unit/moneyParity.test.ts` runs the API, the web copy and
 * this one over the same cases, so editing one without the others fails the
 * build. Keep all three identical.
 *
 * Nothing here decides anything: the server recomputes every total from the
 * line items it receives, and its answer is the one that is stored.
 */
