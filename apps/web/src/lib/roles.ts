 ...
 * one job: deciding which screen to show a signed-in user. It is navigation,
 * not permission. Every screen behind these checks calls an endpoint that asks
 * the server the same question, and a browser that reached a route it should
 * not have gets 403s instead of data. If the two tables ever disagree, the
 * server's answer is the real one.
 *
 * It is a copy rather than an import because the web app does not build from
 * the API's source. Keeping the shape identical is deliberate: when the server
